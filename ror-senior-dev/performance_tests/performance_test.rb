...
puts "Loaded #{@data[:sailings].length} sailings"
puts "Loaded #{@data[:rates].length} rates"
puts "Loaded #{@data[:exchange_rates].keys.length} days of exchange rates"
...
def format_cost(sailing_or_sailings)
  ...
  if sailing_or_sailings.is_a?(Array)
    ...
    sailing_or_sailings.each do |sailing|
      if sailing['rate_eur_cents']
        total_cost += sailing['rate_eur_cents']
        currency = 'EUR'
      elsif sailing['cost_in_cents']
        total_cost += sailing['cost_in_cents']
        currency = sailing['rate_currency']
      elsif sailing['rate']
        # If no cents value is available, convert from rate string
  ...
  elsif sailing_or_sailings['rate_eur_cents']
    # For a single sailing
    "#{sailing_or_sailings['rate_eur_cents'] / 100.0} EUR"
